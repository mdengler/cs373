...
def random_sample(w):
    """QUITE SLOW / MEMORY HUNGRY

returns an index into w representing a choice of that element
according to its probability"""
    sum_w = sum(w)
    normalized = [wi / sum_w for wi in w]
    chosen = 0
    while chosen < len(w):
        if random.random() < normalized[0]:
            return chosen
        else:
            chosen += 1
            normalized.pop(0)
            sum_n = sum(normalized)
            normalized = [ni / sum_n for ni in normalized]
    raise Exception("didn't work: %s / %s" % (chosen, normalized))

p3 = [p[random_sample(w)] for i in range(len(p))]
print p3[:10]
